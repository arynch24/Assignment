...
export default function RescheduleAppointmentModal({
  ...
  const [selectedDate, setSelectedDate] = useState<Date | null>(new Date(appointment?.appointmentDateTime));
  ...
  const handleReschedule = async () => {
    ...
    try {
      setLoading(true);
      const appointmentDateTime = `${selectedDate?.toISOString().split('T')[0]}T${selectedSlot.start}`;
      
      // Only send changed fields
      const updatedAppointment = await appointmentApi.updateAppointment(appointment.id, {
        doctorId: selectedDoctor.id,
        appointmentDateTime,
        // Note: patientId is NOT sent (backend keeps original)
      });
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-60 overflow-y-auto">
                {availableSlots.map((slot, index) => (
                  ...
                  >
                    {slot.start} - {slot.end}
                    {!slot.isAvailable && <span className="ml-1 text-red-500">Booked</span>}
                  </Button>
                ))}
              </div>
            </>
